...
### ✅ Blogcard拡張機能【v1.6.0】
SYN OWND テーマの `[blogcard]` ショートコードに `target` 属性のサポートを追加します。
- **target属性のサポート**
    - `[blogcard url="https://example.com" target="_self"]` のように指定可能
    - `target="_blank"`（新しいタブ）/ `target="_self"`（同じタブ）を自由に選択
- **管理画面で ON/OFF 切り替え可能**
    - デフォルトは OFF です。「Blogcard拡張」メニューから有効化してください。
...
- **JavaScriptによる動的挿入**
    - カード型、リスト型、サムネイル型など、すべてのレイアウトで抜粋表示を実現
    - ページ読み込み後に自動的に抜粋を挿入
- **文字数調整機能**
    - PC表示時の抜粋文字数を設定可能（デフォルト120文字）
    - モバイルは自動的に80%に調整
- **管理画面で ON/OFF 切り替え可能**
    - デフォルトは OFF です。「抜粋表示（JS挿入）」メニューから有効化してください。
- **注意**
    - JavaScript無効環境では動作しません
    - テーマファイルを変更しないため、アップデートの影響を受けません
...
### **1.6.0（2026-02-04）**
**Blogcard拡張機能 & 抜粋表示（JS挿入）を追加**
- Blogcard拡張: `[blogcard]` ショートコードで `target` 属性をサポート
  - `target="_self"` / `target="_blank"` を自由に指定可能
- 抜粋表示（JS挿入）: 記事一覧でどのレイアウトでも抜粋を動的表示
  - カード型、リスト型、サムネイル型などすべてのレイアウトに対応
  - 文字数調整機能付き（デフォルト120文字、モバイルは80%）
  - JavaScriptによる動的挿入でテーマファイルを変更不要
